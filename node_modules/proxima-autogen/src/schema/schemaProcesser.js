...
function processFieldDefinition(field, entityDict = {}, inputText = "") {
  ...
  if (entityDict[name] && inputText == "") {
    name = name + inputText;
    let fieldResolver =
      "  @goField(name:" + name + isList + " , forceResolver: true)";
...
function generateGetAllQuery(entity) {
  let entitiyGetAllQueryString =
    entity.name.value +
    "s(from: [String], where: String, order_by: String, asc: Int, first: Int, last: Int, limit: Int, prove: Boolean): [" +
    entity.name.value +
    "]!\n";
  return entitiyGetAllQueryString;
}
...
function generateTestGetAllQuery(entityName, entityText) {
  let entityGetAllQueryString =
    "query { " +
    entityName +
    "s(from: $ids, where: $whereText, order_by: $order_by, asc: $asc, first: $first, last: $last, limit: $limit, prove: $prove) {" +
    entityText +
    "}}";

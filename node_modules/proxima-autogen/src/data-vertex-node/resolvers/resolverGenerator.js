...
function createEntityResolverFunction(functionText, dataloaders = {}) {
  ...
  if (functionText.includes("[]*models.")) {
    ...
    let body = "return dataloader.For(ctx).$loaderName.LoadThunk(obj.$input)()";
    body = body
      .split("$loaderName")
      .join(loaderName)
      .split("$input")
...
function generateResolverBodyText(functionString) {
  ...
  switch (type) {
    ...
    case "mutation":
      ...
      break;
    case "getAll":
      //(where: String, order_by: String, asc: Int, first: Int, last: Int, limit: Int, prove: Boolean):
      output = getOutput(type, tableName, functionString);
      body +=
        //args from (set defaults)
        //args where
        //args first
        //args last
        //args limit
      ...
      body +=
        ...
        generateValueUnmarshallingText(tableName, "dataRow") +

...
const genAppConfigTestPath = require.resolve(
  "./helpers/gen-app-config-test-template.yml"
);
...
describe("Proxima CLI", async function () {
  describe("init()", async function () {
    ...
    it("should generate the app config", async function () {
      ...
      let actual_app_config = yaml.safeLoad(
        fs.readFileSync(homeDir + "/app-config.yml")
      );
      assert(hasConfig);
  ...
  describe("build()", async function () {
    ...
    it("should generate the data aggregator using the handlers, blockchain client, and proxima client", function () {
      ...
      let hasVertexClient = fs.existsSync("./DataAggregator/generated/models");
      assert(hasVertexClient);
      let hasBlockchainClient = fs.existsSync(
    ...
    //   let hasIndex = fs.existsSync("./generated/index.js");
    //
    //
